...
Module author: Muhammad, 20160727
...
def plot_rti(sTime, rad, eTime=None, bmnum=7, fileType='fitacf',
                    ...
                    plot_terminator=False, data_dict=None):
    ...
    data_dict : Optinal[dict, default value is None]
        This takes the output of read_data function in this module. 
    ...
    if data_dict is None:
        ...
        # Make sure that we only plot data for the time range specified
        selected_indices = [x for x in xrange(len(data_dict['datetime'])) if \
                (data_dict['datetime'][x] >= sTime and data_dict['datetime'][x] <= eTime)]
        sindx = selected_indices[0]
        eindx = selected_indices[-1]
        for keywrd in data_dict.keys():
            data_dict[keywrd] = data_dict[keywrd][sindx:eindx+1]
...
#    # Plot the sky noise.
#    plot_skynoise(skynoise_ax, data_dict['datetime'],
#                  data_dict['nsky'])
#    # Plot the search noise.
#    plot_searchnoise(searchnoise_ax, data_dict['datetime'],
#                        data_dict['nsch'])
    # plot the frequency bar.
    plot_freq(freq_ax, data_dict['datetime'],
                data_dict['freq'])
    # Plot the nave data.
    plot_nave(nave_ax, data_dict['datetime'],
                data_dict['nave'])
    # Plot the cpid bar
    plot_cpid(cpid_ax, data_dict['datetime'],
    ...
    for p in range(len(params)):
        ...
        pos = [.1, figtop - figheight * (p + 1) + .02, .76,
                figheight - .02]
        ax = draw_axes(rti_fig, data_dict['datetime'], rad,
...
#    if np.all(sky):
#        sky_log10 = np.log10(sky)
#    else:
#        sky_log10 = np.array([0]*len(sky))
#    #sky_log10 = [np.log10(x) if x>0 else np.nan for x in sky]
    sky_log10 = np.log10(sky)
    ax.plot_date(date2num(times), sky_log10, fmt='k-',
...
#    if np.all(search):
#        search_log10 = np.log10(search)
#    else:
#        search_log10 = np.array([0]*len(search))
#    search_log10 = [np.log10(x) if x>0 else np.nan for x in search]
    search_log10 = np.log10(search)
    ax.plot_date(date2num(times), search_log10,
...
def rti_panel(ax, data_dict, pArr, gsct, rad, bmnum, coords, cmap,
    ...
    for i in range(len(data_dict['datetime'])):
        x[tcnt] = date2num(data_dict['datetime'][i])
        dt_list.append(data_dict['datetime'][i])

        if(i < len(data_dict['datetime']) - 1):
            if(date2num(data_dict['datetime'][i + 1]) - x[tcnt] > 4. / 1440.):
                tcnt += 1
